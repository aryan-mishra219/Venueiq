...
def send_turn_email(recipient_email, visitor_name, ticket_id):
    ...
    api_secret = get_secret('MAILJET_API_SECRET')
    sender_email = get_secret('SENDER_EMAIL', 'no-reply@venueiq.com')
    
    if not api_key or not api_secret:
        logger.warning(f"Skipping email for {recipient_email}: Mailjet credentials not found in Secret Manager.")
        return False
    ...
    try:
        ...
        logger.error(f"Internal error during email dispatch to {recipient_email}: {str(e)}")
        return False
